...
CONFIDENCE = 0.5
GRADUATED_FLASK_CLASS_NAME = "graduated-flask"
...
def main():
    ...
    print("\n=== Graduated Flask Alignment Check ===")
    ...
    while True:
        ...
        if getattr(result, 'obb', None) is not None:
            ...
            for i, det in enumerate(xywhr):
                ...
                if class_name == GRADUATED_FLASK_CLASS_NAME:
                    flask_points.append((int(x), int(y), normalize_angle_deg(math.degrees(angle))))
            ...
            for i, box in enumerate(boxes):
                ...
                if class_name == GRADUATED_FLASK_CLASS_NAME:
                    flask_points.append((cx, cy, 0.0))
                elif class_name == EYES_CLASS_NAME:
                    eye_points.append((cx, cy))

        if flask_points and eye_points:
            eye_x = sum(p[0] for p in eye_points) / len(eye_points)
            eye_y = sum(p[1] for p in eye_points) / len(eye_points)

            for flask_x, flask_y, angle_deg in flask_points:
                dx = flask_x - eye_x
                dy = flask_y - eye_y
                status, score = classify_alignment(dx, dy, frame.shape[1], frame.shape[0])

                cv2.circle(annotated, (int(flask_x), int(flask_y)), 6, (255, 0, 0), -1)
                cv2.circle(annotated, (int(eye_x), int(eye_y)), 6, (0, 165, 255), -1)
                cv2.line(annotated, (int(flask_x), int(flask_y)), (int(eye_x), int(eye_y)), (0, 255, 255), 2)

                cv2.putText(annotated, f"Flask angle: {angle_deg:.1f}°", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            ...
            cv2.putText(annotated, "Graduated flask or eyes not detected", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)

        cv2.putText(annotated, f"FPS: {fps:.1f}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        cv2.imshow('Graduated Flask Alignment', annotated)
