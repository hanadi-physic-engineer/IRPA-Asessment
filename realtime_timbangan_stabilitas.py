...
# Temporal Pose Stability Analysis (TPSA) untuk stabilitas timbangan.
# Input utama berasal dari YOLO OBB: x, y, w, h, theta, confidence.
MODEL_PATH = (
    "/home/an/lab-detection/IRPA-Asessment/07.ketr-timbangan/"
    "train_07.ketr-timbangan.v1i.yolov8-obb/weights/best.pt"
)
CONFIDENCE = 0.5
IMAGE_SIZE = 640
CAMERA_INDEX = 0
...
TARGET_CLASS_NAME = "analytical-balance"
TARGET_CLASS_KEYWORDS = ("balance", "timbangan", "scale")
OBJECT_DISPLAY_NAME = "Timbangan"
WINDOW_NAME = "Balance Stability - TPSA"
BUFFER_SIZE = 10
MIN_POSES_FOR_ANALYSIS = 4

# Bobot Balance Stability Score (BSS).
...
def find_model_path(path: str) -> str:
    candidates = [
        ...
        "weights/best.pt",
        "train_07.ketr-timbangan.v1i.yolov8-obb/weights/best.pt",
        "IRPA-Asessment/07.ketr-timbangan/train_07.ketr-timbangan.v1i.yolov8-obb/weights/best.pt",
    ]
    ...
    for root, _, files in os.walk("."):
        if "best.pt" in files and "timbangan" in root.lower():
...
def normalize_model_names(names) -> List[str]:
    if isinstance(names, dict):
        return [str(names[key]).strip().lower() for key in sorted(names)]
    if isinstance(names, (list, tuple)):
        return [str(name).strip().lower() for name in names]
    return []


def select_target_class_name(names) -> Optional[str]:
    class_names = normalize_model_names(names)
    for class_name in class_names:
        if any(keyword in class_name for keyword in TARGET_CLASS_KEYWORDS):
            return class_name
...
def extract_best_target_pose(result, target_class_name: Optional[str]) -> Optional[Pose]:
...
def draw_tpsa_panel(
    ...
    if pose is None:
        put_text(image, "TIMBANGAN TIDAK TERDETEKSI", (panel_x + 14, panel_y + 66), 0.58, (0, 0, 255), 2)
        put_text(image, "Buffer direset untuk menjaga validitas TPSA", (panel_x + 14, panel_y + 94), 0.44, (210, 210, 210))
        return

    if result.ready:
        score_text = f"BSS: {result.score:.0f}%"
    else:
        score_text = f"BSS: --  ({buffer_len}/{BUFFER_SIZE})"

    put_text(image, OBJECT_DISPLAY_NAME, (panel_x + 14, panel_y + 58), 0.55, (230, 230, 230))
...
def main() -> None:
    ...
    target_class_name = select_target_class_name(getattr(model, "names", None))
    print(f"Model classes: {normalize_model_names(getattr(model, 'names', None))}")
    print(f"Target class: {target_class_name or 'all detected OBB objects'}")

    print("\n=== Balance Stability Detection (YOLO OBB + TPSA) ===")
    ...
    try:
        while True:
            ...
            pose = extract_best_target_pose(result, target_class_name)
            if pose is None:
                pose_buffer.clear()
